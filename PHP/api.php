<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
        ...
        if(isset($_GET['id'])){
            ...
            $id = $_GET['id'];
            $sql = $con->query("select * from empresa where id='$id'");
            $data = $sql->fetch_assoc();
        }else{
            // Entra nesse, caso não tenha passagem de ID via "get"
            $data = array();
            $sql = $con->query("select * from empresa");
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        // gravar informações
        $data = json_decode(file_get_contents("php://input"));
        $sql = $con->query("insert into empresa(Nome, Telefone, Email, CNPJ ,Senha) values ('".$data->nome."','".$data->Telefone."','".$data->Email."','".$data->CNPJ."','".$data->Senha."')'");
        if($sql){
